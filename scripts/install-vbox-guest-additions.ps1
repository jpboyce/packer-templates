...
$current_checksum = (Get-FileHash -Path $iso_path -Algorithm SHA256).hash.tolower()
...
if ($current_checksum -eq $iso_checksum) {
  ...
  Mount-DiskImage -ImagePath $iso_path -StorageType ISO -NoDriveLetter -Verbose
  $drive = gwmi win32_volume -Filter "Label = 'VBox_GAs_5.2.22'"
  $drive.DriveLetter = "X:"
  $drive.Put()
